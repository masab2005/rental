...
export default function Maintenance(){
  ...
  function cancelEdit(){
    setEditingId(null);
    setEditForm({ maintenancetype: '', maintenancedate: '', maintenancecost: '' });
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-black text-slate-900 mb-2">Maintenance Records</h2>
        <p className="text-slate-600">Track and manage vehicle maintenance activities</p>
      </div>

      {loading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-slate-600 text-lg">Loading maintenance records...</div>
        </div>
      ) : items.length ? (
        <div className="grid gap-6">
          {items.map(it => (
            <div key={it.maintenanceid} className="group bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-slate-200">
              {editingId === it.maintenanceid ? (
                // Edit Mode
                <div className="space-y-4">
                  <div className="text-lg font-bold text-slate-900 mb-4">Edit Maintenance Record</div>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditForm({...editForm, maintenancedate: e.target.value})}
                      className="w-full px-4 py-2 border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Cost (Rs)</label>
                    <input
                      type="number"
                      value={editForm.maintenancecost}
                      onChange={(e) => setEditForm({...editForm, maintenancecost: e.target.value})}
                      className="w-full px-4 py-2 border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-700">Car ID:</span>
                        <span>{it.carid}</span>
                      </div>
                      <div className="h-4 w-px bg-slate-300"></div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-700">Date:</span>
                        <span>{new Date(it.maintenancedate).toLocaleDateString()}</span>
                      </div>
                      <div className="h-4 w-px bg-slate-300"></div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-700">Cost:</span>
                        <span className="text-lg font-bold text-emerald-600">Rs{it.maintenancecost}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <div className="text-xl font-bold text-slate-900 mb-2">No Maintenance Records</div>
          <div className="text-slate-600">All vehicles are up to date with maintenance</div>
        </div>
      )}
    </div>
  );
